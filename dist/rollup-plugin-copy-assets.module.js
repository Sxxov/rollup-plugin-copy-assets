  ...
  return {
    ...
    async generateBundle({
      ...
      return Promise.all(assets.map(async asset => {
        try {
          const assetChildPath = asset.replace(path.basename(basedir), "");
          const target = path.join(outputDirectory, assetChildPath);
          const normalizedAssetPath = path.normalize(basedir);
          const assetPath = path.join(normalizedAssetPath, assetChildPath);
          const targetIsDir = path.extname(target) === "";
          const targetExists = await lib.pathExists(target);

          if (targetIsDir) {
            if (targetExists) await lib.emptyDir(target);
          } else {
            if (targetExists) {
              await lib.mkdirs(path.dirname(target));
              await lib.remove(target);
            }
          }

          return await lib.copy(assetPath, target, {
            overwrite: true,
            errorOnExist: false
          });
        } catch (e) {
